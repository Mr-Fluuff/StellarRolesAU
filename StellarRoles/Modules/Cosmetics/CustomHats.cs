...
namespace StellarRoles.Modules.Cosmetics
{
    ...
    public class CustomHats
    {
        ...
        {
            Texture2D texture = fromDisk ? Helpers.LoadTextureFromDisk(path) : CustomAssets.CosmeticsBundle.LoadAsset<Texture2D>(path);
            if (texture == null)
                return null;
        ...
        private static HatData CreateHatBehaviour(CustomHat ch, bool fromDisk = false, bool testOnly = false)
        {
            ...
            if (ch.Animation != null)
            {
                ...
                {
                    extend.Animation.Add(CreateHatSprite(frame, fromDisk));
                }
                extend.Time = 0;
                extend.Frame = 0;
            }
            if (ch.BackAnimation != null)
            {
                ...
                foreach (string frame in ch.BackAnimation)
                    extend.BackAnimation.Add(CreateHatSprite(frame, fromDisk));
                extend.Time = 0;
                extend.Frame = 0;
            }
            if (testOnly)
        ...
        private static HatData CreateHatBehaviour(CustomHatLoader.CustomHatOnline chd)
        {
            ...
            chd.Resource = chd.Resource;
            if (chd.BackResource != null)
                chd.BackResource = chd.BackResource;
            if (chd.ClimbResource != null)
                chd.ClimbResource = chd.ClimbResource;
            if (chd.FlipResource != null)
                chd.FlipResource = chd.FlipResource;
            if (chd.BackflipResource != null)
                chd.BackflipResource = chd.BackflipResource;

            return CreateHatBehaviour(chd, false);
        }
            ...
            public static void SetHat(PlayerPhysics __instance, HatViewData viewData)
            {
                ...
                if (extend.Animation?.Count > 0 || extend.BackAnimation?.Count > 0)
                {
                    if (extend.Animation?.Count > 0)
                    {
                        ...
                        if (extend.Frame == 5)
                        {
                            Helpers.Log(extend.Frame + " - " + extend.Animation[extend.Frame].name);
                        }
                    }
        ...
        public class HatsTabOnEnablePatch
        {
            ...
            public static float CreateHatPackage(List<(HatData, HatExtension)> hats, string packageName, float YStart, HatsTab __instance)
            {
                ...
                if (textTemplate != null)
                {
                    ...
                    titleText.fontSize = 5f;
                    titleText.fontSizeMax = 5f;
                    titleText.fontSizeMin = 0f;
